...
A useful section of the MultiQC output is in the STAR section which shows a bar plot with alignment rates. As in the [nf-core/rnaseq](https://github.com/nf-core/rnaseq) pipeline: "good samples should have most reads as _Uniquely mapped_ and few _Unmapped_ reads".
...
- `salmon/suppa/`
  ...
  - `split_files/per_isoform/` or `split_files/per_local_event/` -`*.psi`: Contains relative abundance value per sample for each event (split by sample group)
  - `split_files/tpms/`
