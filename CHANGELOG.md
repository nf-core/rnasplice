...
- #258 - Read the samplesheet only once. `RMATS` receives the samplesheet channel instead of a second `file(params.input)`, and the rMATS single condition check reuses the parsed rows instead of reading the CSV with `new File`/`new URL`, which only handled local and `http` paths (by @piplus2)
