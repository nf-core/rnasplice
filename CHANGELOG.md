...
- #258 - Added pipeline level nf-tests for `--source genome_bam`, `--source transcriptome_bam` and `--source salmon_results`, none of which had any test coverage (by @piplus2)
...
- #258 - Fixed `--source genome_bam`, `--source transcriptome_bam` and `--source salmon_results` aborting at startup. `PIPELINE_INITIALISATION` validated every samplesheet against `assets/schema_input.json`, the fastq schema, so a samplesheet for any other source failed with `Missing required field(s): strandedness, fastq_1` before a single process was submitted. The schema is now selected from `--source` by `samplesheetSchema()`, which is the one place that maps a source to its schema (by @piplus2)
- #258 - The samplesheet is now read once, in `PIPELINE_INITIALISATION`, and the parsed channel is carried through `NFCORE_RNASPLICE` into `RNASPLICE`. `RNASPLICE` no longer parses `params.input` a second time, `RMATS` receives the samplesheet channel instead of a second `file(params.input)`, and the rMATS single condition check no longer reads the CSV by hand with `new File`/`new URL`, which only handled local and `http` paths (by @piplus2)
- #258 - Fixed `BAM_SORT_STATS_SAMTOOLS` being called with a bare fasta path in the `genome_bam` and `transcriptome_bam` paths, which aborted the run with `Input tuple does not match tuple declaration in process ... SAMTOOLS_SORT`. It now receives the `[ meta, fasta, fai ]` tuple the subworkflow expects (by @piplus2)
- #258 - Fixed `No such variable: ch_txi_suppa_tpm` in the `salmon_results` path with `--suppa`. The SUPPA TPM now comes from `TX2GENE_TXIMPORT_SALMON.out.suppa_tpm`, as the variable it referenced is only assigned in the `star_salmon` branch (by @piplus2)
- #258 - Fixed `ISOFORMSWITCHANALYZER` being handed the `.tar.gz` archives instead of the extracted Salmon directories with `--source salmon_results`, which failed with `There were no directories containing the file names/suffixes typically generated by Kallisto/Salmon/RSEM/StringTie`. `TX2GENE_TXIMPORT` already extracts them internally and now emits them (by @piplus2)
