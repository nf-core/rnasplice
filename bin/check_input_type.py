...
if all(col in samplesheet.columns and not samplesheet[col].isna().all() for col in ['fastq_1', 'fastq_2']):
    ...
    samplesheet[fastq_columns].to_csv(args.samplesheet_file + '.fastq.csv', index=False)
    ...
    if 'transcriptome' in samplesheet.columns and not samplesheet['transcriptome'].isna().all():
        ...
        samplesheet[transcriptome_columns].to_csv(args.samplesheet_file + '.transcriptome.csv', index=False)
    else:
        # Output the bam column set
        samplesheet[bam_columns].to_csv(args.samplesheet_file + '.bam.csv', index=False)
elif 'salmon' in samplesheet.columns and not samplesheet['salmon'].isna().all():
    # Output the salmon column set
    samplesheet[salmon_columns].to_csv(args.samplesheet_file + '.salmon.csv', index=False)
else:
